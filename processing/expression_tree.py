...
class ExpressionTree(object):
    def __init__(self):
        self.ops_list = [';', 'SEP', '=', '+', '-', '*', '/', '^','//','round','min','max','power','combination','permutation']
        self.ops_priority = {";": 0, "SEP": 0, "=": 1, "+": 2, "-": 2, "*": 3, "/": 3, "^": 4, '//':3, 'round':4,'min':4,'max':4,'power':4,'combination':4,'permutation':4}
        self.root = None
    ...
    def _infix(self, et_node):
        if et_node == None:
            ...
            if et_node.value in self.ops_list: #["+", "-", "*", "/", "^", '=']:
                ...
                # else:
                if et_node.left and et_node.left.value in ["+", "-", "*", "/", "=","^","%","round","min","max","power","combination","permutation"] and self.ops_priority[et_node.value] > self.ops_priority[et_node.left.value]:
                    left_list = ['('] + self._infix(et_node.left) + [')']
                else:
                    left_list = self._infix(et_node.left)

                # if et_node.right and et_node.value == "-" and et_node.right.value == "+":
                if et_node.right and et_node.right.value in ["+", "-", "*", "/", "=","^","%","round","min","max","power","combination","permutation"] and self.ops_priority[et_node.value] >= self.ops_priority[et_node.right.value]:
                    right_list = ['('] + self._infix(et_node.right) + [')']
                else:
                    right_list = self._infix(et_node.right)
